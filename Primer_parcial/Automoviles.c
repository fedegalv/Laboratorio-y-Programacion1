...
int copiarVehiculosHistorial(sAutomovil listaAutomoviles[], sAutomovil historialAutos[], int tam)
{
    int i;
    for (i= 0; i < tam; i++)
    {
...
int buscarAudi(sAutomovil listaAutos[], int tam)
{
    int idEncontrado;
    int i;
    for (i= 0; i < tam; i++)
    {
        if(listaAutos[i].estado == 1 && listaAutos[i].marca == AUDI)
        {
            idEncontrado= listaAutos[i].idPropietario;
            break;
        }
    }
